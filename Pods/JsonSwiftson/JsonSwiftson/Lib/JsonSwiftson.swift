...
public final class JsonSwiftson {
  ...
  public init(anyObject: AnyObject?, parent: JsonSwiftson) {
    self.parent = parent
    parsedRawValue = anyObject
  }

  //
  // Map JSON value to a non-object value. It can be a string, number, boolean, null or an array.
  //
  // Arguments
  // ----------
  //
  ...
  // withClosure
  //
  //    Supplying a closure is useful together with optional: true. That will allow mapping JSON
  //    to an optional Swift structure.
  //
  public func map<T>(optional: Bool = false, withClosure closure: ((JsonSwiftson)->(T?))? = nil) -> T? {
